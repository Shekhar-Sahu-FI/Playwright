...
export class FormOperation<T = Record<string, any>> {
  ...
  private readonly saveDataFn: (page: Page, data: T, mode?: 'save' | 'update' | '', deleteData? : boolean, request? : any) => Promise<void>;

  constructor(
    page: Page,
    formLayout: FormLayout,
    saveData: (page: Page, data: T, mode?: 'save' | 'update' | '', deleteData? : boolean, request? : any) => Promise<void>,
  ...
  async saveAndVerify(data: T, request:any) {
    await test.step('Create and save new record', async () => {
      await this.openNewForm();
      await this.saveDataFn(this.page, data, 'save', true, request);
    });
  }
