...
const SaveData = async (page: Page, data: any, mode: "save" | "update" | "" = "") => {
    await test.step("Fill the form", async () => {
        ...
        await itemMasterPage.fillSubgroupName(data.itemName);
        await itemMasterPage.fillGroup(data.itemSubgroupQuery, data.itemSubgroupName);
        ...
        if (data.isUnitConversion) {
            ...
            for (let i = 0; i < data.itemMasterUnitConvertionDetail.length; i++) {
                const { unitType, unitConvertionType, baseUnitValue, unitConvertionNameQuery, unitConvertionName, unitConvertionValue } = data.itemMasterUnitConvertionDetail[i];
