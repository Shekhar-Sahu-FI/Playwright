...
test.describe('Item group Master save with excel', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const newPath = 'master/item-group-master';
    ...
    for (let i = 0; i < rows.length; i++) {
      await test.step(`Row Index : ${i + 1} ${rows[i]['Group Name']}`, async () => {
        try {
          ...
          await Promise.race([
            SaveData(page, {
              code: String(rows[i]['Group Code']).slice(2, 4),
              name: rows[i]['Group Name'].trim(),
...
const SaveData = async (page: Page, data: any) => {
  await test.step('Fill the form', async () => {
    await itemGroupMasterPage.selectCategory(data.categoryName);
