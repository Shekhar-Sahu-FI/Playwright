...
const SaveData = async (page: Page, data: any, mode: 'save' | 'update' | '' = '') => {
  await test.step('Fill the form', async () => {
    await costCenterMasterPage.fillBusinessUnit(data.businessUnitQuery, data.businessUnitName);
    await expect(costCenterMasterPage.businessUnit).toHaveValue(data.businessUnitName);

    await costCenterMasterPage.fillCode(data.code);
    await expect(costCenterMasterPage.code).toHaveValue(data.code);

    await costCenterMasterPage.fillCostCenterName(data.name);
    // await costCenterMasterPage.fillParentCostCenter(data.parentQuery, data.parentCostCenterName);
    await costCenterMasterPage.fillParentCostCenter("Cos", "Cost Cen");
    await expect(costCenterMasterPage.parentCostCenter).toHaveValue("Cost Center 20");

    await costCenterMasterPage.fillDescription(data.description);
    await expect(costCenterMasterPage.description).toHaveValue(data.description);
