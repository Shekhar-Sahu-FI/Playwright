...
test.describe("User Master Full API Test", () => {
  ...
  test("Update Owner Type", async () => {
    const getRes = await getAndSearch(`${baseURL}/Search`, {
      userProfileId: "Admin",
    });
    const updatedData = {
      ...getRes[0],
    ...
    expect(
      response.validationErrors[0].PropertyName == "UserTypeNo"
    ).toBeTruthy();
    expect(
  ...
  test("Update Owner Email", async () => {
    ...
    const response = await res.json();
    expect(
      response.validationErrors[0].PropertyName == "EmailId",
      "Checking Email Field"
    ).toBeTruthy();
    expect(
      response.validationErrors[0].ErrorMessage ==
        "Email Id of owner cannot be changed.",
  ...
  test("Delete Owner Type", async () => {
    ...
    expect(
      res.validationErrors[0].ErrorMessage == "Owner Cannot be deleted."
    ).toBeTruthy();
  });
...
const validateGetRes = (formData, getRes) => {
  expect(
    getRes.userProfileId == formData.userProfileId,
    "userProfileNo check"
  ).toBeTruthy();
  expect(getRes.UserName == formData.UserName, "userName check").toBeTruthy();
  expect(
    getRes.UserTypeNo == formData.UserTypeNo,
    "userTypeNo check"
  ).toBeTruthy();
};
...
const validateSearchRes = (formData, searchRes) => {
  expect(
    searchRes.userTypeNo == formData.userTypeNo,
    "userTypeNo check"
  ).toBeTruthy();
  expect(
    searchRes.userProfileId == formData.userProfileId,
    "userProfile check"
  ).toBeTruthy();
  expect(
    searchRes.userName == formData.userName,
    "userName check"
  ).toBeTruthy();
  expect(searchRes.emailId == formData.emailId, "emailId check").toBeTruthy();
  expect(
    searchRes.contactNo == formData.contactNo,
    "conatactNo check"
  ).toBeTruthy();
  expect(
    searchRes.statusNo == formData.statusNo,
    "statusNo check"
  ).toBeTruthy();
};
...
function validateResponseMatch(formData: any, apiResponse: any) {
  expect(
    apiResponse.userProfileId == formData.userProfileId,
    "userProfile check"
  ).toBeTruthy();
  expect(
    apiResponse.userTypeNo == formData.userTypeNo,
    "userTypeNo check"
  ).toBeTruthy();
  expect(
    apiResponse.userName == formData.userName,
    "userName check"
  ).toBeTruthy();
  expect(apiResponse.emailId == formData.emailId, "emailId check").toBeTruthy();
  expect(
    apiResponse.employeeId == formData.employeeId,
    "employeeId check"
  ).toBeTruthy();
  expect(
    apiResponse.designation == formData.designation,
    "designation check"
  ).toBeTruthy();
  expect(
    apiResponse.reportingManagerName == formData.reportingManagerName,
    "reportingManagerName check"
  ).toBeTruthy();
  expect(
    apiResponse.status?.statusNo == formData.statusNo,
    "statusNo check"
  ).toBeTruthy();
  expect(
    apiResponse.statusRemarks == formData.statusRemarks,
    "statusRemarks check"
  ).toBeTruthy();
}
